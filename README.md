...
## ⚠️ WARNING:
- Use DELETE, TRUNCATE, and DROP commands very carefully on a live/production database.
- DELETE removes selected records based on the WHERE condition.
- TRUNCATE removes all records from the table.
- DROP removes the entire table, including its structure and data.
- Always verify the command and take a backup before running it on a live database.
...
-- Delete the entire contact table
DROP TABLE contact;
```
...
SELECT name, salary FROM contact ORDER BY salary DESC LIMIT 3;

-- Second Highest Salary employee
SELECT name, salary FROM contact ORDER BY salary DESC LIMIT 1 OFFSET 1; 
```
...
SELECT EXTRACT(YEAR FROM created_at) FROM qtymast;
```
